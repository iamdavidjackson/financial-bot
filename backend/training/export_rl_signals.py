...
from training.train_rl_signal_lstm import (
    FORECAST_HORIZON,
    MODEL_PATH,
    SCALERS_PATH,
    WINDOW_SIZE,
...
DOWNLOAD_END = (pd.Timestamp.today().normalize() + pd.Timedelta(days=1)).strftime(
    "%Y-%m-%d"
)
...
def export() -> pd.DataFrame:
    ...
    for ticker in PORTFOLIO_TICKERS:
        ...
        if df.empty or len(df) < required_rows:
            raise ValueError(
                f"{ticker}: only {len(df)} rows, need at least {required_rows}"
            )
        ...
        print(
            f"{ticker}: {len(return_series)} walk-forward predictions "
    ...
    return pd.DataFrame(
        {
            ...
            "mean_predicted_return": [
                predicted_returns_df[t].mean() for t in PORTFOLIO_TICKERS
            ],
